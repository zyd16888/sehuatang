...
日志页保留文件选择，新增可组合的级别和模块筛选。`crawler.log` 包含普通、警告及错误，
`error.log` 只包含 ERROR 和 CRITICAL；级别筛选按选中的级别精确匹配。
模块包括 Sehuatang、JavBee、x1080x、Telegram、调度、数据库与存储、系统，以及未分类历史日志。
例如选择 `crawler.log`、`ERROR`、`x1080x`，即可查看 x1080x 的错误记录。
筛选不会改变服务的写入级别；默认 `logging.level: INFO` 不生成 DEBUG 日志。

新日志格式为 `时间 - 级别 - [module=x1080x] 消息`。模块由调用代码所属组件或显式绑定确定，
公共抓取引擎、HTTP 重试和 CF 客户端绑定对应来源；数据库/台账操作归为数据库与存储，
不会仅因为正文提到某来源就改归属。历史日志没有模块标记时仍可按级别查看，模块归为未分类，
不重写旧文件。新格式在更新程序并重启后生效。

页面“条数”表示最近匹配的日志记录数，异常堆栈随原记录一起显示，实际文本行数可以更多。
`GET /api/logs` 支持 `file`、`lines`、`level`、`module` 参数；从所选当前文件尾部向前查找，
每次最多扫描 4 MiB，不搜索轮转备份文件。到达扫描上限时页面会提示，更早的日志可能未包含。
筛选后的条数不受原先“先取最近 200 行”限制；自动刷新保留当前筛选和阅读时的滚动位置。
...
Sehuatang 按页补抓会读取分页栏中明确的末页链接或总页数提示，达到真实末页后
切换下一分区；邻近数字链接的最大值不作为总页数。没有分页栏时继续逐页检查，
只有正常列表结构里的空主题列表才视为到底。

勾选续跑或起始页大于 1 时，先读取第一页核对边界。例如旧检查点为 1000、实际末页为 50，
会只读取第一页后跳过该分区，不再请求 1001～2000。旧检查点保留原值，不会自动改写或重置；
日志和本轮汇总的 `last_page` 显示已确认的实际末页。

越界响应明确显示真实末页时正常结束；无法确认越界的整页重复、页码错位、权限/登录/验证页、
主题字段解析不完整，以及已知末页之前突然出现空列表，都标记为 `list_failed@页码`，
记录具体原因并保留此前检查点，其他分区继续处理。整页重复以普通主题 tid 集合判断，
正常的部分重叠或“这一页的帖子都已入库”不会单独触发停止。
